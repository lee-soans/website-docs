...
description: Learn how to configure a Cloudflare-managed domain for use with WordPress Hosting.
---
...
Because of some of its features, there are some steps you'll have to follow to add any Cloudflare managed domain to your site in WordPress Hosting:

1. Ensure your DNS Records (relevant A and CNAME records) are configured as [explained here](https://help.wordpress.hosting/?p=12).
2. In the "Crypto" tab of Cloudflare change the "SSL" to **Full**
3. In the "Crypto" tab of Cloudflare change "Always use HTTPS" to **Off**
4. In WordPress Hosting, add the domain(s) in the Domains tab. Note: There might be some propagation delay for those records. See the [DNS Record Caching](https://help.wordpress.hosting/?p=4) article for more information about that.
